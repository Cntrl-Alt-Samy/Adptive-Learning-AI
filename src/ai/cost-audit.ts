...
/**
 * Minimal structural SQL executor — pg.Pool in production, pg.Client (or any
 * query-shaped adapter) in tests. Only `.query` is required.
 */
...
export class PgAuditSink implements AuditSink {
  ...
  async listAudits(sessionId?: string): Promise<AiExecutionAuditRow[]> {
    const sql = sessionId
      ? {
          text: `SELECT session_id::text, model_used, prompt_cache_hit, input_tokens, output_tokens, cost_gbp, latency_ms
                 FROM ai_execution_audits WHERE session_id = $1::uuid ORDER BY created_at`,
          values: [sessionId] as unknown[]
        }
      : {
          text: `SELECT session_id::text, model_used, prompt_cache_hit, input_tokens, output_tokens, cost_gbp, latency_ms
    ...
    const res = await this.db.query(sql.text, sql.values);
    return (res.rows as Array<Record<string, unknown>>).map(
      (r) =>
        ({
          sessionId: r.session_id as string,
          modelUsed: r.model_used as string,
          promptCacheHit: r.prompt_cache_hit as boolean,
          inputTokens: r.input_tokens as number,
          outputTokens: r.output_tokens as number,
          costGbp: r.cost_gbp as number,
          latencyMs: r.latency_ms as number
        }) satisfies AiExecutionAuditRow
    );
  }
}
